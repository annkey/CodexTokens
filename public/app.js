...
function renderInsights(data) {
  ...
  const insightCards = [
    {
      label: "日环比",
      value: formatPercent(analytics.dayOverDay),
      sub: "相较昨日总用量",
      trend: analytics.dayOverDay,
    },
    {
      label: "周环比",
      value: formatPercent(analytics.weekOverWeek),
      sub: "相较上周总用量",
      trend: analytics.weekOverWeek,
    },
    {
      label: "月环比",
      value: formatPercent(analytics.monthOverMonth),
      sub: "相较上月总用量",
      trend: analytics.monthOverMonth,
    },
    {
      label: "7日平均用量",
      value: formatCompact(analytics.movingAverage7d),
      sub: "",
    },
  ];
...
async function loadDashboard() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(payload.details || payload.error || "加载失败");
    }
